#!/usr/bin/env python3
"""
Highway No.1 (좌측 1 m 이동 경로) — nominal vs residual. v=12 와 v=20.
...
이후라, 잔차가 6.5~6.9 Hz 로 돌던 기존 주행과 달리 제어율이 맞는 비교다.
  ...
  주행   48 t, Q_n=1e-4, 잔차 scale 0.3, 6차원 모델(재학습 없음)
  결과   v=12, v=20 모두 양쪽 완주 (925~926 m)

**v=20 은 학습 상한(12.23 m/s)의 +67% 외삽이다.** 실제 도달 속도가 20.4 m/s 로,
대형 트럭의 실주행 속도대다. 925 m 경로에서 정지 출발부터 가속하므로 평균 속도는
14.2 m/s 이고, v >= 15 구간이 329 샘플(약 33 초)이다. 그 구간만 떼어낸 지표도
함께 저장한다(met_hi) - 고속만 보면 개선폭이 더 크다.
...
def build(v_target, f_nom, f_res, out, pi, s_ref, x_ref, y_ref, psi, kap, kthr, kf):
    ...
    M = np.zeros((2, 8)); MH = np.zeros((2, 8))
    for i, (lbl, f) in enumerate(runs):
        ...
        for row, w in ((M, s >= S_MIN), (MH, (s >= S_MIN) & (v >= V_HI))):
            if not w.any():          # v=12 주행에는 v>=15 샘플이 없다
                row[i] = np.nan
                row[i, 6], row[i, 7] = 1.0 / dt, s.max() - s.min()
                continue
            c = w & (np.abs(kf(s)) >= kthr)
            row[i] = [np.sqrt((n[w] ** 2).mean()), np.abs(n[w]).mean(), np.abs(n[w]).max(),
                      np.rad2deg(np.sqrt((al[w] ** 2).mean())),
                      np.rad2deg(np.abs(al[w]).max()),
                      np.sqrt((n[c] ** 2).mean()) if c.any() else np.nan,
                      1.0 / dt, s.max() - s.min()]
    ...
    for tag, row in (("전 구간 (s>=%.0f)" % S_MIN, M),
                     ("고속 구간 (v>=%.0f)" % V_HI, MH)):
        if np.isnan(row[0, 0]):
            print(f"  [{tag}]  해당 샘플 없음")
            continue
        print(f"  [{tag}]")
        print(f"{'':>14}" + "".join(f"{h:>17}" for h in hdr[:6]))
        for i in range(2):
            print(f"{runs[i][0]:>14}" + "".join(f"{row[i,j]:17.3f}" for j in range(6)))
        print(f"{'개선':>14}"
              + "".join(f"{100*(row[0,j]-row[1,j])/row[0,j]:16.1f}%" for j in range(6)))
    print(f"  제어율 {M[0,6]:.1f} / {M[1,6]:.1f} Hz,   거리 {M[0,7]:.0f} / {M[1,7]:.0f} m")
    ...
    for key, src in (("traj_s", TS), ("traj_n", TN), ("traj_a", TA),
        ...
        for i in range(2):
            arr[i] = src[i]
        D[key] = arr
    path = os.path.join(HERE, "matlab", out)
