...
| `plot_mass_summary.m` | **적재별 종합** — 예측 오차 × 주행 오차 × 본/안 본 적재 4칸. 안 본 적재 주행은 주행 전까지 빈 칸 (창3: 횡/헤딩/표) |
...
| `plot_control_inputs.m` | **제어 입출력** — 창2개: 명령(스로틀/조향각/조향각속도) + 응답(속도/가속도). `SIGNALS`/`XAXIS` 로 신호·축 선택 |
