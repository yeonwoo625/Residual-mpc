...
%   횡 dn    조건화 이득 -19.6 / +1.3 / -25.8 / -43.4 %   (12쌍 중 10쌍이 무게 X 승)
%   헤딩 da  조건화 이득 -54.7 / +2.8 / -22.0 / -108.9 %  (12쌍 중  9쌍이 무게 X 승)
%
% 기전은 플라시보가 말해준다. **섞은무게가 무게 O 를 11/12(횡), 10/12(헤딩) 이긴다.**
% 그리고 섞은무게는 무게 X 와 거의 같다(횡 차이 +0.00012 m). 무게 열이 학습 때
% 잡음이면 신경망은 그 열을 무시하는 법을 배우고, 무시하니까 안 본 적재에서도
% 멀쩡하다. 반대로 진짜 무게를 주면 신경망은 본 네 값에 맞춘 사상을 배우고, 다섯
% 번째 값이 들어오면 그 사상이 깨진다. 시드 산포도 무게 O 가 7.1배(횡) / 6.6배
% (헤딩) 크다 - 안 본 값에서 불안정하다.
%
...
% [0.2, 0.1, 0.05, 0.4] 로 클램프된 뒤 MPC 로 간다. 안 본 적재에서는 MLP 가 범위
% 밖으로 튀어 클램프가 실제로 작동하므로, 클램프 없는 원출력으로 재면 오차가
% 과장된다(56 t 횡 -43.4% 가 -112.5% 로 보인다). plot_mass_conditioning.m 도 같은
% 경로를 쓰므로 두 표는 서로 비교 가능하다.
%
% 창 2개가 뜬다.
