...
%   안 본 적재 예측에서 조건화가 **진다**. 횡 -19.6 / +1.3 / -25.8 / -43.4 %.
%              12 시드쌍 중 10쌍이 무게 X 승. 플라시보가 11/12 로 조건화를 이긴다.
%              주행에서도 56 t 조건화 이득이 11.3% -> 0.4% 로 사라진다.
%
...
%
% ** 안 본 적재 주행 ** 56 t 는 있다 (results/heldout56/, 2026-08-04, 3시드).
% 그때 쓴 모델이 lomo_mass.py 의 56 t 모델과 가중치·정규화까지 동일함을 확인했다.
% 32/40/48 t 는 아직 없어서 '(not run yet)' 이 찍힌다. 채우려면:
%   ./scripts/run_ablation.sh lomo_nomass 48 0   (TruckMaker Loads=16000)
%   ./scripts/run_ablation.sh lomo_cond   48 0
%
% ** 제어율 ** 주행은 (c)(d) 모두 6.3~6.5 Hz 다(솔버 수정 전). 같은 조건이라
% (c) 대 (d) 비교가 성립한다.
%
% 창 3개가 뜬다.
