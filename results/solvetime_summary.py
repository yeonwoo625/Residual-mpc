...
원인의 원인 (2026-09-03 추가): l4acados 의 solve() 에는 수렴 판정 분기가 있으나
acados 옵션 rti_log_residuals 에 연동돼 있고 기본값이 0 이라 실행되지 않았다.
즉 "조기 종료가 없다"가 아니라 "옵션이 꺼져 있었다"가 정확하다. 반면 acados 자체
SQP 는 조기 종료하므로 nominal 은 반복 상한을 20 으로 올려도 0.41 ms 로 동일하다.
따라서 7배 차이는 "잔차가 더 많이 최적화해서"가 아니라 "수렴 후에도 멈추지 않아서"다.

옵션을 켜고 재보면 (offline, N=20, 같은 x0 반복):
  Cython + 조기종료 끔 (현재 배포)  9.05 ms,  실제 10 회
  ctypes + 조기종료 끔              16.70 ms, 실제 10 회
  ctypes + 조기종료 켬               2.11 ms, 실제  1 회   <- 4.3배 단축
  ctypes + 조기종료 켬, 상한 100     2.13 ms, 실제  1 회
이 acados 버전의 Cython 인터페이스에는 get_initial_residuals 가 없어 조기 종료를
쓰려면 ctypes 빌드가 필요하다. ctypes 는 호출 오버헤드로 느리지만(16.70) 9 회를
안 도는 이득이 훨씬 크다.

주행 기본값은 끈 상태로 둔다 - 켜면 10 회 -> 1 회로 해가 달라져 기존 주행을 전부
재검증해야 한다. 잔차 주입의 실제 추가 비용은 SQP 1 회당 0.28 ms 다.
